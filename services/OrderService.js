...
function doCheckOrderParams(params) {
  return new Promise(function(resolve, reject) {
    ...
    if (!params.order_id) {
      if (!params.user_id) return reject("客户ID不能为空");
      if (isNaN(parseInt(params.user_id))) return reject("客户ID必须是数字");
...
function doUpdateOrder(info) {
  return new Promise(function(resolve, reject) {
    dao.update("OrderModel", info.order_id, _.clone(info), function(
      err,
      newOrder
    ) {
...
module.exports.getOrder = function(orderId, cb) {
  if (!orderId) return cb("客户ID不能为空");
  if (isNaN(parseInt(orderId))) return cb("客户ID必须是数字");
